...
export default function ProductCardGroup(props:Props) {
    ...
    let renderCard = ()=>{
        return props.value? <Card id="product-card-group-container" className={"card-container shadow p-3 mb-5 bg-white rounded"}>
            <Card.Body>
                ...
                <Card.Text style={{display:"flex"}}> 貨存 : {props.value.has_stock? <div style={{marginLeft:"1rem"}}><FontAwesomeIcon icon={solid("circle")} style={{color: "#c9ffc2",}} /> 有</div>:
                    <div style={{marginLeft:"1rem"}}><FontAwesomeIcon icon={solid("circle")} style={{color: "#ff0000",}} /> 沒有</div>}</Card.Text>
                ...
                <div style={{display:"flex",justifyContent:"space-between"}}>
                    ...
                    <Button variant="outline-primary" onClick={()=>{
                        if(user){
                            ...
                            window.alert(`1件 ${props.value.name} 已加入購物車`)
                        }else {
                            navigate("/login")
                        }
